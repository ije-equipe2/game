...
{
    audio::play_sound_effect("/usr/share/deadly_wish/sound/fx/infiltrador_heavy.ogg", EFFECTS_VOLUME, 0);
    m_active = false;
...
Infiltrator::light_attack() {
    audio::play_sound_effect("/usr/share/deadly_wish/sound/fx/infiltrador_light.ogg", EFFECTS_VOLUME, 0);
    auto p = parent();
    printf("p = %p\n", (void *) p);
...
void
Infiltrator::defense() {
    audio::play_sound_effect("/usr/share/deadly_wish/sound/fx/infiltrador_block.ogg", EFFECTS_VOLUME, 0);
    change_character_state(DEFENSE_STATE);
}

void
Infiltrator::special() {
    audio::play_sound_effect("/usr/share/deadly_wish/sound/fx/infiltrador_ultimate.ogg", EFFECTS_VOLUME, 0);
